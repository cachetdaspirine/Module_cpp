    ...
    */
    ...
    if(TopOrLeft){
        if(Horizontal){// top
          for(int i=0;i<Lx;i++){
            if((i+Ly-1)%2==0){ // down
              Index.push_back({i,Ly-1,7});
              Index.push_back({i,Ly-1,1});
              Index.push_back({i,Ly-1,11});
              Index.push_back({i,Ly-1,5});
            }
          }
        }
        else{//left
          for(int i = 0; i<Ly;i++){
            if(i%2==0){ // down
              ...
              Index.push_back({0,i,7});

              Index.push_back({1,i,2});
              Index.push_back({1,i,8});
            }
          }
        }
    }
      ...
      if(Horizontal){ // bottom
        for(int i=0;i<Lx;i++){
          if((i)%2==1){ // Up
            Index.push_back({i,0,8});
            Index.push_back({i,0,2});
            Index.push_back({i,0,4});
            Index.push_back({i,0,10});
          }
        }
      }
      else{//right
        for(int i = 0; i<Ly;i++){
          if((i+Ly-1)%2==0){ // down
            Index.push_back({Ly-1,i,11});
            Index.push_back({Ly-1,i,5});
          }
          else if((i+Ly-1)%2==1){
            Index.push_back({Ly-1,i,6});
